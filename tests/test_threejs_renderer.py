...
class ThreeJSRendererTests(unittest.TestCase):
    ...
    def test_threejs_renderer_exposes_opt_in_sky_debug_instrumentation(self):
        ...
        self.assertIn("function ovizDebugSnapshot()", html)
        self.assertIn("snapshot: ovizDebugSnapshot", html)
        self.assertIn("root.__OVIZ_DEBUG__ = window.__OVIZ_DEBUG__;", html)
        self.assertIn('root.dataset.ovizDebug = ovizDebugEnabled ? "true" : "false";', html)
        self.assertIn("root.dataset.ovizDebugSnapshot = JSON.stringify({", html)
    ...
    def test_threejs_renderer_exposes_sky_aperture_for_live_aladin_backgrounds(self):
        ...
        self.assertIn("let skyApertureFrameLastViewSignatures = [];", html)
        self.assertIn("skyApertureFrameLastViewSignatures[safeIndex] = \"\";", html)
        self.assertIn("function skyAperturePostedFrameViewSignature(frameIndex, view)", html)
        ...
        self.assertIn("const framePairsToSend = framePairs.filter(({ index }) => {", html)
        self.assertIn("return frameSignature !== skyApertureFrameLastViewSignatures[index];", html)
        self.assertIn("if (!framePairsToSend.length) {", html)
        self.assertIn("skyApertureFrameLastViewSignatures[index] = skyAperturePostedFrameViewSignature(index, view);", html)
