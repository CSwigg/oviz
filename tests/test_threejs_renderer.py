...
class ThreeJSRendererTests(unittest.TestCase):
    ...
    def test_threejs_renderer_exposes_sky_aperture_for_live_aladin_backgrounds(self):
        ...
        self.assertNotIn("skyBackgroundPredictiveTransformForAlignedView(alignedView, currentView)", html)
        self.assertNotIn("frameEl.style.transform = transform;", html)
        self.assertNotIn("const view = options && options.view ? options.view : skyDomeBackgroundViewForCamera();", html)
        self.assertNotIn("const minUpdateIntervalMs = skyDomeBackgroundUserCameraActive ? 16.0 : 120.0;", html)
        ...
        self.assertIn("function createSkyAperturePresetFrame(index) {\n        return null;\n      }", html)
        self.assertIn("function ensureSkyAperturePresetFrames(options = {}) {\n        return [];\n      }", html)
        self.assertNotIn('frameEl = document.createElement("iframe");', html)
        self.assertNotIn("oviz-three-sky-aperture-frame-stack", html)
        self.assertNotIn("oviz-three-sky-aperture-frame-preset", html)
        self.assertNotIn("function skyApertureActiveFrameIndexes(", html)
        self.assertNotIn("skyAperturePresetOptions.forEach((_, index) => createSkyAperturePresetFrame(index));", html)
        self.assertNotIn("const activeFrameIndexes = new Set(skyApertureActiveFrameIndexes(blend));", html)
        self.assertNotIn("frameEls.forEach((frameEl) =>", html)
        self.assertNotIn("if (!syncAllFrames && !activeFrameIndexes.has(frameIndex)) {", html)
        self.assertNotIn("activeFrameIndexes: Array.from(activeFrameIndexes)", html)
        self.assertNotIn("clipEl.style.opacity = String(Math.min(Math.max(opacity, 0.0), 1.0));", html)
        ...
        self.assertIn("postSkyApertureLayerStateToBaseFrame(blend)", html)
        self.assertIn("postSkyApertureClipToBaseFrame(clip)", html)
